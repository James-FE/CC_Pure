...
export async function sideQuery(opts: SideQueryOptions): Promise<BetaMessage> {
  ...
  if (opts.querySource === 'auto_mode') {
    logForDebugging(
      `[sideQuery] auto_mode parentSpan=${parentSpan ? `id=${(parentSpan as unknown as Record<string, unknown>).id ?? 'present'}` : 'null/undefined'} querySource=${opts.querySource}`,
    )
  }

  // When parentSpan is provided, create a child span nested under the
  // main agent trace. For auto_mode queries, never create a standalone
  // root trace; those observations should appear within the parent.
  const langfuseTrace = parentSpan
